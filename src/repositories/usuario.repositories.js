...
export const PostUsuario = (correo,nombre,contrasena) => {
    return new Promise((resolve, reject) => {
       const consQuery = "INSERT INTO usuario (correo,nombre,contrasena) values (?,?,?)";
      database
        .execute(consQuery, [correo,nombre,contrasena])
        .then((result) => {
          resolve(result);
        })
        .catch((err) => reject(err));
